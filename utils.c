...
void my_exec(char *cmd) {
  ...
  while (token && np < 255) {
    ...
    {
      char *env_var =
          getenv(token + 1); // Извлечение значения переменной окружения
      if (env_var) {
        params[np++] = env_var;
      } else {
  ...
  if (IS_CMD(pwd))
    ...
    SHCMD_EXEC1(cd);
  else { // иначе вызов внешней команды
...
// параметры: строка команды, количество команд в конвейере, текущая (начинается
// с 0 )
int exec_conv(char *cmds[], int n, int curr) {
  ...
  if (n > 1 && curr < n - 2) { // first n-2 cmds
    if (fork() == 0) {
      dup2(fd[1], 1);
      close(fd[0]);
      close(fd[1]);
  ...
  } else { // 2 last cmds or if only 1 cmd
    if (n == 1) {
      if (strstr(cmds[0], "cd") != NULL) {
        // Закрытие файловых дескрипторов пайпа, так как они больше не нужны в
        // данном процессе
        close(fd[0]);
        close(fd[1]);
        // printf("%s \n", cmds[curr]);
    ...
    if (n == 1 &&
        strcmp(cmds[0], "exit") == 0) { // для случая команды exit обходимся без
                                        // fork, иначе не сможем завершиться
      close(fd[0]);
      close(fd[1]);
      my_exec(cmds[curr]);
      return 0;
